...
public class MyHashMap implements Map {
   ...
   public int size() {
    ...
    public void addEntry(Object key, Object value, int hashValue, int i) {
        ...
        /*
         * 上面两句话，需要注意的是，如果i位置原始位置table[i]没有值，eNode为空，就直接将新的node放入table[i],并将该node的next指向空
         * 否则，有值的话，该位置上新增一个node，形成一个链表结构，新的node插入到原始node的前面，所以新的node的next指向原始位置i的node
         */
    }
